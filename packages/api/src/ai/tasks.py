"""Celery task that bridges Django (eu-central-1) to the generate_image Lambda (us-west-2).

The Lambda is invoked synchronously (RequestResponse) so we get image keys
back in the same call. The worker updates the GenerationJob row in-place;
the frontend polls `GET /api/ai/jobs/<id>/`.
"""
